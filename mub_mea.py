...
if __name__ == '__main__':
    ...
    #     case "MUB":
    #         import sympy as sp
    #         # --------------generate 5 dimensional MUB--------------------
    #         Identity5 = sp.eye(5)
    #         # omega = exp(2*pi*I/5)
    #         omega = sp.Symbol('omega')
    #         omega_num = np.exp(2*math.pi*1j/5)
    #         dephased_complex_Hadamard = sp.Matrix([[1,1,1,1,1],[1,omega,omega**2,omega**3,omega**4],[1,omega**2,omega**4,omega,omega**3],[1,omega**3,omega,omega**4,omega**2],[1,omega**4,omega**3,omega**2,omega]])

    #         f5 = sp.lambdify(omega, dephased_complex_Hadamard, 'numpy')
    #         f5_num = 1/math.sqrt(5)*f5(omega_num)
    #         diagU = sp.diag(1,omega,omega**4,omega**4,omega)

    #         H5_1 = 1/sp.sqrt(5)*sp.Matrix([[1,1,1,1,1],[omega,omega**2,omega**3,omega**4,1],[omega**4,omega,omega**3,1,omega**2],[omega**4,omega**2,1,omega**3,omega],[omega,1,omega**4,omega**3,omega**2]])
    #         # print(H5_1)

    #         H5_2 = 1/sp.sqrt(5)*sp.Matrix([[1,1,1,1,1],[omega**2,omega**3,omega**4,1,omega],[omega**3,1,omega**2,omega**4,omega],[omega**3,omega,omega**4,omega**2,1],[omega**2,omega,1,omega**4,omega**3]])
    #         # print(H5_2)

    #         H5_3 = 1/sp.sqrt(5)*sp.Matrix([[1,1,1,1,1],[omega**3,omega**4,1,omega,omega**2],[omega**2,omega**4,omega,omega**3,1],[omega**2,1,omega**3,omega,omega**4],[omega**3,omega**2,omega,1,omega**4]])
    #         # print(H5_3)

    #         H5_4 = 1/sp.sqrt(5)*sp.Matrix([[1,1,1,1,1],[omega**4,1,omega,omega**2,omega**3],[omega,omega**3,1,omega**2,omega**4],[omega,omega**4,omega**2,1,omega**3],[omega**4,omega**3,omega**2,omega,1]])
    #         # print(H5_4)

    #         # print(diagU**3)
    #         F5 = 1/sp.sqrt(5)*dephased_complex_Hadamard
    #         # print(F5)

    #         MUB5 = []
    #         MUB5.append(sp.eye(5))
    #         MUB5.append(F5)
    #         MUB5.append(diagU*H5_1)
    #         MUB5.append(diagU**2*H5_2)
    #         MUB5.append(diagU**3*H5_3)
    #         MUB5.append(diagU**4*H5_4)
    #         # print(MUB5)
    #         # print(type(MUB5))

    #         MUB5_FUN = sp.lambdify(omega, MUB5, 'numpy')
    #         MUB5_NUM = np.array(MUB5_FUN(omega_num))
    #         # print(type(MUB5_NUM))
    ...
    #             sync_mub = MUB5_NUM[i]
    #             for j in range(0,5):
    #                 chal1_mub = MUB5_NUM[j]
    #                 for k in range(0,5):
    #                     sync_basis = sync_mub[:,k]
    #                     if i == 0:
    #                         AMP_1 = [0,0,0,0,0]
    #                         AMP_1[k] = 1
    #                     else:
    #                         AMP_1 = [1,1,1,1,1]
    #                     PHA = np.angle(sync_basis).tolist()
    #                     SYNC_HOLY = {'Amp':AMP_1,'Pha':PHA,'Topo':TOPO}
    #                     # TODO: refresh_1
    #                     TH260P.window.refresh_1(SYNC_HOLY, direct_call = True)
    #                     time.sleep(0.1)
    #                     for l in range(0,5):
    #                         chal1_basis = chal1_mub[:,l]
    #                         if j ==0:
    #                             AMP_2 = [0,0,0,0,0]
    #                             AMP_2[l] = 1
    #                         else:
    #                             AMP_2 = [1,1,1,1,1]
    #                         PHA = np.angle(chal1_basis).tolist()
    #                         CHAL1_HOLY = {'Amp':AMP_2,'Pha':PHA,'Topo':TOPO}
    #                         # TODO: refresh_2
    #                         TH260P.window.refresh_2(CHAL1_HOLY, direct_call = True)
    #                         # TODO: do measurement and save data
    #                         time.sleep(0.1)
    #                         #------------------------------------------
    #                         COLLECT_TIME = int(5.5*1000)       # int:[ms]  how many times to collect data
    #                         COLLECT_INTERVAL = 1               # [s]   interval between two collect
    ...
    #                         TH260P.poData2GUI(([syncrate, chan1rate, ccrate, hrate[-1]]))
    #                         # TODO: save the data
    #                         with open(data_path + '\\' + 'sync_' + str(i) + str(k) + 'chal1_' + str(j) + str(l) + '.txt','w', encoding = 'utf-8') as f:
    #                             f.write(json.dumps(["SYNC_HOLY:", "MUB:", i, "BASIS:", k, SYNC_HOLY]))
    #                             f.write('\t')
    #                             f.write(json.dumps(["CHAL1_HOLY:", "MUB:", j, "BASIS:", l, CHAL1_HOLY]))
    #                             f.write('\n')
    ...
    #                             # with open('./mea_base_mub.txt','a', encoding = 'utf-8') as f:
    #                             #         f.write(json.dumps(["SYNC_HOLY:", "MUB:", i, "BASIS:", k, SYNC_HOLY]))
    #                             #         f.write('\t')
    #                             #         f.write(json.dumps(["CHAL1_HOLY:", "MUB:", j, "BASIS:", l, CHAL1_HOLY]))
    #                             #         f.write('\n')
